...
const dice = (num) => (num = Math.round(Math.random() * 6));
console.log(dice())
/* ESERCIZIO 2
...
console.log("indirizzo email?", isThisAnEmail("gaetanodeleonibus@hotmail.it"));
...
const howManyDays = (giornitrascorsi) => {
  let today = new Date();
  let dateTo = new Date(giornitrascorsi);
  let timeDiff = today.getTime() - dateTo.getTime();
  return Math.floor(timeDiff / (1000 * 60 * 60 * 24));
};
console.log("giorni trascorsi", howManyDays("1997-02-22"), "trascorsi da tale data.");
...
const newestMovie = (arrey) => {
  let filmpiurecente = arrey[0];

  arrey.forEach((element) => {
    if (parseInt(filmpiurecente.anni) < parseInt(element.anni)){
